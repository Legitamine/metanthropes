...
## Early Access v0.14.xxx [2026-07-xx]
...
- [Core] TODO Split the Narrator Journal into smaller ones for easier reading & to improve page navigation and linking.
...
- First pass on overhauling the Macros. From now on, instead of Macros having script code, they now reside under the new `metanthropes.macros` API, allowing for future iterrations without the need to replace the Macro itself.
...
- All Macros are now consolidated under the new `metanthropes.macros` API. Old Macros are obsolete and should no longer be used. Import the updated Macros from their respective Compendiums.
