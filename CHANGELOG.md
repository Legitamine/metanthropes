...
- TODO Added a new `Play Cutscene Macro`, available under the new `metanthropes.macros.playCutscene({ cutsceneUUID, cutsceneDurationInSeconds })` API. This new Macro triggers a synchronous playback of a Cutscene for all connected Clients and returns to the previously active Scene once the duration expires. See this blog? on how to use - link to blog? quick video with how-to [] use it for your own Cutscenes.
