...
-   Possessions now display the Material Properties tab, previously only visible to Homebrew ownwers. Added tooltips to better clarify what each property does. This tab displays the material properties of the item, the Resistances it provides to the wearer (if an Armor type) and other properties, such as the item's Durability, Size and Weight. For Armors, the Resistances values provided, are not automatically applied to the Actor and the automation will come in future builds. Narrators will have to edit the Actor's Resistances Score directly, to give them the armor's protection bonuses.
