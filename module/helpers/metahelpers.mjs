...
export async function metaIsMetapowerEquipped(actor, metapower) {
	...
	return equippedItems.some((item) => item.system.MetapowerName === metapower);
}

/**
 * Helper to quickly get if setting of a Module is enabled
 *
 * @export
 * @param {string} module
 ...
 * @returns {boolean}
 */
export function metaCheckSetting(module, setting) {
	const active = game.modules.get(`metanthropes-${module}`)?.active ?? false;
	if (!active) return false;
	return game.settings.get(`metanthropes-${module}`, setting) ?? false;
}
