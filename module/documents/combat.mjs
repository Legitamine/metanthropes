...
export class MetanthropesCombat extends Combat {
			...
			if (bleedingLevel > 0) {
				...
				if (metaHomebrew) {
					...
					if (homebrewBleeding !== 1)
						combatantMessage += `Lost ${lifeLoss} <i class="fa-duotone fa-solid fa-heart"></i> Life due to ${homebrewName} for Bleeding Condition ${bleedingLevel}.<br>`;
					else combatantMessage += `Lost ${lifeLoss} <i class="fa-duotone fa-solid fa-heart"></i> Life due to Bleeding Condition ${bleedingLevel}.<br>`;
				} else {
					lifeLoss = Number(bleedingLevel);
					newLife = Number(currentLife) - lifeLoss;
					combatantMessage += `Lost ${lifeLoss} <i class="fa-duotone fa-solid fa-heart"></i> Life due to Bleeding Condition ${bleedingLevel}.<br>`;
				}
	...
	// 				await ChatMessage.create({
	// 					content: `Lost ${bleedingLevel} <i class="fa-duotone fa-solid fa-heart"></i> Life due to Bleeding Condition ${bleedingLevel}.<br><br>`,
	// 					speaker: ChatMessage.getSpeaker({ actor: actor }),
	// 				});
	// 			}
	// 		}
