...
export async function metaHungerRoll(actor, hungerLevel) {
	...
	hungerMessage = `Rolls to beat Hunger <i class="fa-sharp-duotone fa-solid fa-skull"></i> Condition Level ${hungerLevel} and gets a result of ${hungerRollResult} (needed ${hungerTarget} or less).<br><br>`;
	if (hungerRollResult > hungerTarget) {
		hungerMessage += `It is a <i class="fa-sharp-duotone fa-solid fa-square-xmark"></i> Failure!<br><br>${actor.name} is too hungry and can't act!`;
		//? Button to re-roll Hunger using destiny
		const currentDestiny = Number(actor.system.Vital.Destiny.value);
		hungerMessage += `<hr />${actor.name} has ${currentDestiny} <i class="fa-sharp-duotone fa-solid fa-hand-fingers-crossed"></i> Destiny remaining.<br>`;
		if (currentDestiny > 0) {
		...
		hungerMessage += `It is a <i class="fa-sharp-duotone fa-solid fa-square"></i> Success!<hr />${actor.name} has overcome Hunger!<br><br>`;
