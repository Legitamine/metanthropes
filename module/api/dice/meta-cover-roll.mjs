...
export async function metaCoverRoll(actor, coverType, coverValue) {
	...
	if (coverRollResult > coverTarget) {
		coverMessage += `It is a <i class="fa-duotone fa-solid fa-square-xmark"></i> Failure!<br><br>${actor.name} can't find Cover!`;
		//? Button to re-roll Cover using destiny
		const currentDestiny = Number(actor.system.Vital.Destiny.value);
		coverMessage += `<hr />${actor.name} has ${currentDestiny} <i class="fa-duotone fa-solid fa-hand-fingers-crossed"></i> Destiny remaining.<br>`;
		if (currentDestiny > 0) {
		...
		coverMessage += `It is a <i class="fa-duotone fa-regular fa-square"></i> Success!<hr />${actor.name} found ${coverType} Cover!<br><br>`;
	}
	coverRoll.toMessage({
